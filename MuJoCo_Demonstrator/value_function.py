...
class NNValueFunction(object):
    ...
    def __init__(self, env_name, obs_dim, hid1_mult):
        """
        Args:
    ...
    def _build_graph(self):
        ...
        with self.g.as_default():
            ...
            out = tf.layers.dense(out, 1,
                                  kernel_initializer=tf.random_normal_initializer(
                                      stddev=np.sqrt(1 / hid3_size)), name='h4')
            self.out = tf.squeeze(out, name='output_valfunc')
    ...
    def close_sess(self):
        """ Save and close TensorFlow session """
        model_directory = './saved_models/' + self.env_name + '/valfunc/'
